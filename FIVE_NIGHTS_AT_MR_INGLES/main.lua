...
local function drawCameraAnim(a, now, camName)
    ...
    if sprite then
        ...
        local sx = baseScale * (game.width / 1280) * (1 + wobble)
        local sy = baseScale * (game.height / 720) * (1 + wobble)
        local iw, ih = sprite:getWidth(), sprite:getHeight()
        love.graphics.draw(sprite, a.x, a.y + wobble * 30, wobble, sx, sy, iw / 2, ih / 2)
        return
    end

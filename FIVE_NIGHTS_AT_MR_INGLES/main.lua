...
local game = {
    ...
    hour = 12,
    hourTimer = 0,
    secondsPerHour = 60,     -- change for shorter/longer nights
    width = 1280,
    height = 720,
    status = "",
}
...
local function drawOfficeAnim(a, now)
    -- Only draw threats that can be seen from the office view
    if a.room ~= "Hallway" and a.room ~= "Office" then return end

    local sprite = getAnimSprite(a.name)
    if sprite then
        local iw, ih = sprite:getWidth(), sprite:getHeight()
        local wob = math.sin(now * 2 + a.x * 0.01) * 0.02
        local scale = 0.4
        local sx = scale * (game.width / 1280) * (1 + wob)
        local sy = scale * (game.height / 720) * (1 + wob)

        love.graphics.setColor(1, 1, 1)
        love.graphics.draw(sprite, a.x, a.y + wob * 40, wob, sx, sy, iw / 2, ih / 2)
    else
...
local function drawOfficeOverlays()
    -- Doors overlay the office (left and right) - ALWAYS draw these last

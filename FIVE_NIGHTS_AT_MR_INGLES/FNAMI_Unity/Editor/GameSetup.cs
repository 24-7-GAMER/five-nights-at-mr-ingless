...
public class GameSetup
{
    ...
    private static void BuildMainMenuScene()
    {
        ...
        CreateRootGameObject("GameManager", "FiveNightsAtMrIngles.GameManager");
        CreateRootGameObject("AudioManager", "FiveNightsAtMrIngles.AudioManager");
        CreateRootGameObject("SaveLoadManager", "FiveNightsAtMrIngles.SaveLoadManager");
        ...
        ConfigureAudioManager(GameObject.Find("AudioManager")?.GetComponent<Component>());
        ...
        // Splash panel
        GameObject splashPanel = CreateUIElement("SplashPanel", canvasObj.transform, "Panel");
        Image splashImage = splashPanel.GetComponent<Image>();
        splashImage.color = Color.white;
        splashImage.sprite = LoadSprite("intro_splashscreen");
        splashImage.preserveAspect = true;

        GameObject fadeOverlay = CreateUIElement("SplashFade", splashPanel.transform, "Panel");
        Image fadeImage = fadeOverlay.GetComponent<Image>();
        fadeImage.color = Color.black;

        GameObject tosToggleObj = CreateUIElement("ToSToggle", splashPanel.transform, "Panel");
        RectTransform tosRect = tosToggleObj.GetComponent<RectTransform>();
        tosRect.anchorMin = new Vector2(0.25f, 0.1f);
        tosRect.anchorMax = new Vector2(0.25f, 0.1f);
        tosRect.sizeDelta = new Vector2(30f, 30f);
        tosRect.anchoredPosition = Vector2.zero;
        Toggle tosToggle = tosToggleObj.AddComponent<Toggle>();
        Image tosBg = tosToggleObj.GetComponent<Image>();
        tosBg.color = new Color(0f, 0f, 0f, 0f);
        GameObject checkmark = new GameObject("Checkmark");
        RectTransform checkRect = checkmark.AddComponent<RectTransform>();
        checkRect.SetParent(tosToggleObj.transform, false);
        checkRect.anchorMin = Vector2.zero;
        checkRect.anchorMax = Vector2.one;
        Image checkImg = checkmark.AddComponent<Image>();
        checkImg.color = new Color(0.6f, 1f, 0.6f, 1f);
        tosToggle.graphic = checkImg;

        GameObject tosLabel = CreateUIText("ToSLabel", splashPanel.transform, new Vector2(55f, -6f), new Vector2(700f, 40f), TextAnchor.MiddleLeft);
        Text tosText = tosLabel.GetComponent<Text>();
        tosText.text = "I agree to the Terms of Service";

        GameObject tosHint = CreateUIText("ToSHint", splashPanel.transform, new Vector2(0f, -60f), new Vector2(800f, 40f), TextAnchor.MiddleCenter);
        Text tosHintText = tosHint.GetComponent<Text>();
        tosHintText.text = "Click the checkbox to continue";

        Component splashCtrl = AddComponent(splashPanel, "FiveNightsAtMrIngles.UI.SplashController");
        SetObject(splashCtrl, "splashImage", splashImage);
        SetObject(splashCtrl, "fadeOverlay", fadeImage);
        SetObject(splashCtrl, "tosToggle", tosToggle);
        SetObject(splashCtrl, "tosLabel", tosText);
        SetObject(splashCtrl, "tosHint", tosHintText);
        SetObjectArray(splashCtrl, "splashSprites", new List<Object>
        {
            LoadSprite("intro_splashscreen"),
            LoadSprite("splash_truestory"),
            LoadSprite("tos_splash")
        });
        SetFloatArray(splashCtrl, "fadeInDurations", new float[] { 1f, 0.5f, 1f });
        SetFloatArray(splashCtrl, "holdDurations", new float[] { 1f, 1f, 3.5f });
        SetFloatArray(splashCtrl, "fadeOutDurations", new float[] { 1f, 0.5f, 1f });
        SetValue(splashCtrl, "tosStageIndex", 2);
    ...
    private static void BuildOfficeScene()
    {
        ...
        // Intro overlay
        GameObject introPanel = CreateUIElement("IntroPanel", canvasObj.transform, "Panel");
        Image introBg = introPanel.GetComponent<Image>();
        introBg.color = Color.black;

        GameObject introTextObj = CreateUIText("IntroText", introPanel.transform, Vector2.zero, new Vector2(1000f, 120f), TextAnchor.MiddleCenter);
        Text introText = introTextObj.GetComponent<Text>();
        introText.fontSize = 48;
        introText.color = Color.white;

        GameObject introFade = CreateUIElement("IntroFade", introPanel.transform, "Panel");
        Image introFadeImg = introFade.GetComponent<Image>();
        introFadeImg.color = Color.black;

        Component introCtrl = AddComponent(introPanel, "FiveNightsAtMrIngles.UI.IntroController");
        SetObject(introCtrl, "introPanel", introPanel);
        SetObject(introCtrl, "introText", introText);
        SetObject(introCtrl, "fadeOverlay", introFadeImg);
    ...
    private static GameObject CreateCamera(string name)
    {
        ...
        EnsureUniversalAdditionalData(cameraObj);
        return cameraObj;
    }

    private static void EnsureUniversalAdditionalData(GameObject cameraObj)
    {
        var dataType = System.Type.GetType(
            "UnityEngine.Rendering.Universal.UniversalAdditionalCameraData, Unity.RenderPipelines.Universal.Runtime");
        if (dataType != null && cameraObj.GetComponent(dataType) == null)
        {
            cameraObj.AddComponent(dataType);
        }
    }
    ...
    private static GameObject CreateRootGameObject(string name, string scriptName)
    {
        GameObject obj = new GameObject(name);
        AddComponent(obj, scriptName);
        return obj;
    ...
    private static void ConfigureAudioManager(Component audioManager)
    {
        ...
        SetObject(audioManager, "doorKnockSFX", LoadAudioClip("door_knock", "Assets/Audio/SFX"));
        SetObject(audioManager, "doorDamageSFX", LoadAudioClip("door_damage", "Assets/Audio/SFX"));
        ...
        SetObject(audioManager, "introMsgSFX", LoadAudioClip("intro_msg", "Assets/Audio/SFX"));
        SetObject(audioManager, "niceTrySFX", LoadAudioClip("NICE_TRY", "Assets/Audio/SFX"));
        SetObject(audioManager, "ventCrawlSFX", LoadAudioClip("vent_crawl", "Assets/Audio/SFX"));
        SetObject(audioManager, "faaahSFX", LoadAudioClip("faaah", "Assets/Audio/SFX"));
    ...
    private static void SetValue(Component component, string propertyName, int value)
    {
        if (component == null) return;
        SerializedObject so = new SerializedObject(component);
        var prop = so.FindProperty(propertyName);
    ...
    private static void SetFloatArray(Component component, string propertyName, float[] items)
    {
        if (component == null) return;
        SerializedObject so = new SerializedObject(component);
        var prop = so.FindProperty(propertyName);
